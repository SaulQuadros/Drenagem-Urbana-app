...
if menu == "Características da Bacia":
    ...
    if st.button('📄 Gerar Relatório Word - Parâmetros Bacia'):
        ...
        # Inserindo os dados do projeto como as primeiras informações da página, com label e valor em negrito
    ...
    if st.button("📄 Gerar Relatório Word - Microdrenagem"):
        if (st.session_state.tc is None or
            ...
            # Inserindo os dados do projeto como as primeiras informações da página, em itens com bullet e com ambos os textos em negrito
            ...
            for item in dados_projeto:
                if ":" in item:
                    ...
                    run_label = p_item.add_run(f"{label}:")
                    run_label.bold = True
                    run_label.font.size = Pt(11)
                    run_label.font.name = 'Aptos'
                    run_value = p_item.add_run(f" {value.strip()}")
                    run_value.font.size = Pt(11)
                    run_value.font.name = 'Aptos'
